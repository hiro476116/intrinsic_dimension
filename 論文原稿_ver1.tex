...
\]
More precisely, $\mathcal T_T$ is given $q_T$-almost everywhere by
...
We next bound the initial transportation cost. Let
...
\begin{equation}
    ...
    \label{eq:M-def}
\end{equation}
is a square-integrable $(\mathcal F_t)$-martingale. Moreover, for
$0\le s\le t\le t_N$,
\begin{equation}
    \E\left[\norm{M_t-M_s}^2\right]
    =
...
\begin{proof}
By the construction in the preceding subsection,
\[
    \mathcal F_t
    =
    \mathcal G_t
...
\begin{split}
    \mathcal F_t
    &=
    \sigma\{X_{T-s}:0\le s\le t\}\\
    &=
    \sigma\{X_u:T-t\le u\le T\}.
\end{split}
\]
The Markov property of the forward Ornstein--Uhlenbeck process
implies that, conditionally on $X_{T-t}$, the variable $X_0$ and the
future path $(X_u)_{T-t\le u\le T}$ are conditionally independent.
Therefore,
\[
    \E\left[
        X_0
...
\]
Using $Y_t=X_{T-t}$ and the definition of the posterior mean, we
obtain
\[
\begin{split}
    \E\left[X_0\mid\mathcal F_t\right]
...
\end{split}
\]
which proves \eqref{eq:martingale-conditional}.

Since $X_0$ is square-integrable and
\[
    M_t=\E\left[X_0\mid\mathcal F_t\right],
\]
the process $(M_t)_{0\le t\le t_N}$ is a square-integrable
$(\mathcal F_t)$-martingale.

Now let $0\le s\le t\le t_N$. We have
\[
    X_0-M_s
    =
    (X_0-M_t)+(M_t-M_s).
\]
Moreover, $M_t-M_s$ is $\mathcal F_t$-measurable and
\[
    \E\left[X_0-M_t\mid\mathcal F_t\right]=0.
\]
Hence
\[
\begin{split}
...
Finally, for every $r\in[0,t_N]$,
\[
    M_r=\E[X_0\mid Y_r].
\]
Therefore, by the definition of conditional covariance,
\[
\begin{split}
    \E\left[\norm{X_0-M_r}^2\right]
    &=
...
\end{split}
\]
Since $Y_r=X_{T-r}$ almost surely,
\[
    \Cov(X_0\mid Y_r)
    =
    \Sigma_{T-r}(Y_r),
...
\]
Substituting this identity with $r=s$ and $r=t$ into the preceding
orthogonal decomposition proves
