...
\begin{assumption}
    \begin{equation}
        \|s_{T-t_n}(X_{T-t_n})-\hat{s}_{T-t_n}(X_{T-t_n})\|_2\leqq \epsilon(t_i)<\infty\quad (0\leqq i \leqq N-1)
    \end{equation}
\end{assumption}
主定理の証明に使う補題を用意する.
\begin{lemma}
...
\begin{proof}
    \begin{align}
        W_2(p_{data}, \mathcal{L}(\hat{Y}_{t_N}))&\leqq E[\|Y_T-\hat{Y}_{t_N}\|^2]^{\frac{1}{2}}=\|Y_T-\hat{Y}_{t_N}\|_2
    \end{align}
    である. ここで, 補題2.3を使うと
    \begin{align}
        \hat{Y}_{t_{n+1}}
...
\begin{proof}
    ...
    よって等長性より, 
    \begin{align}
        \frac{1-\sigma^2_{T-t}}{\sigma^4_{T-t}}\mathbb{E}[\|\mu_{T-t}(Y_t)-\mu_{T-s}(Y_s)\|^2_2]&=2\int_{s}^{t}\mathbb{E}[\|\nabla^2\log{q_{T-u}(Y_u)+\frac{1}{\sigma^2_{T-u}I_d}}\|^2_F]du
    \end{align}
    となるので, 
    \begin{align}
            \frac{d}{dt}(\frac{1-\sigma^2_{T-t}}{\sigma^4_{T-t}}\mathbb{E}[\|\mu_{T-t}(Y_t)-\mu_{T-s}(Y_s)\|^2_2])=2\mathbb{E}[\|\nabla^2\log{q_{T-t}(Y_t)+\frac{1}{\sigma^2_{T-t}}I_d}\|^2_F]
    \end{align}
    ここで, 次の補題を用いる.
    \begin{lemma}
        \begin{equation}
