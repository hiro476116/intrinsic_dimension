...
\end{frame}
 %2
  ...
  \begin{frame}{拡散モデルについて : 生成とは}
    \begin{itemize}
      \item 未知の確率測度 $P$ に従って独立に得られたデータ $\{x_1,\cdots, x_n\}$ があるとする.
      \item このデータを用いて, $P$ に従う新たなデータを近似的に生み出すことを\textbf{生成}という.
    ...
    \node at ($(data.south)!0.5!(model.south)+(0,-0.55cm)$) {
        $P \approx \widehat{P}$
      };
  ...
  \begin{frame}{拡散モデルについて : 拡散モデルの原理}
    ...
    \end{align}
    \begin{itemize}
        \item 上記のSDEの解 $\{Y_t\}_{t\in[0,T]}$ で, 容易にサンプリングが得られる正規分布などを初期分布 ($Y_0\sim N(0,I)$) としたときに, $Y_T\sim P$ となるような $b, \sigma$ を構成できないだろうか？
        \item そのような $b,\sigma$ が得られた場合, (1) をシミュレーションすることで $P$ からのサンプリングが無数に得られる. 
    \end{itemize}
  \end{frame}
  %------------4-----------
  \begin{frame}{拡散モデルについて : 拡散過程の時間反転構造}
    ...
    \end{equation}
    \begin{itemize}
      \item (2)の解を $\{X_t\}_{t\in[0,T]}$ とし, $X_t$ の密度を $p_t$ とする. このとき, 次の結果が知られている. 
    \end{itemize}
  ...
  \begin{frame}{拡散モデルについて : 拡散過程の時間反転構造}
    \begin{itemize}
        \item 画像からノイズへの画像などを貼って直感的に説明するページにする
    \end{itemize}
  \end{frame}
  %---------6-------------
  \begin{frame}{拡散モデルについて : 拡散過程の時間反転構造}
