...
\end{frame}
 ...
 %2
  ...
  \begin{frame}{拡散モデルについて : 生成とは}
    \begin{itemize}
      \item 未知の確率測度 $P_{\text{data}}$ に従って独立に得られたデータ $\{x_1,\cdots, x_n\}$ があるとする.
  ...
  \begin{frame}{拡散モデルについて : 拡散モデルの原理}
    ...
    \end{align}
    \begin{itemize}
        \item 上記のSDEの解 $\{Y_t\}_{t\in[0,T]}$ で, 標準正規分布を初期分布, つまり $Y_0\sim N(0,I)$ としたときに, $Y_T\sim P$ となるような $b, \sigma$ を構成できないだろうか？
    \end{itemize}
  ...
  \begin{frame}{拡散モデルについて : 拡散過程の時間反転構造}
    ...
    \begin{theorem}[\cite{anderson82}]
      ...
      \begin{equation}
        \left\{ \,
            \begin{aligned}
            & dY_t = (Y_t+\nabla \log{p_{T-t}(Y_t)})dt+\sqrt{2}dW_t \quad (0\leqq t < T) \\
  ...
  \begin{frame}{拡散モデルについて : 拡散過程の時間反転構造}
    \begin{itemize}
        \item $dX_t=-X_tdt+\sqrt{2}dW_t$ の解 $\{X_t\}_{t\in[0,T]}$ は, $Z$ を $X_0$ と独立な標準正規分布に従う
        確率変数としたとき, $X_t\stackrel{\mathrm{d}}{=}e^{-t}X_0+\sqrt{1-e^{-2t}}Z$ と表せる. よって,
        $T$ を十分大きく取ると $X_T\approx \mathcal{N}(0,I)$ となることが期待できる.
        \item $X_0\sim P_{\text{data}}$ とすると, $\{X_t\}_{t\in[0,T]}$ はデータ分布から時間の経過とともに標準正規分布へ近づいていく確率過程となる.
    \end{itemize}
  ...
  \begin{frame}{拡散モデルについて : 拡散過程の時間反転構造}
    \begin{itemize}
        \item $\{X_t\}_{t\in[0,T]}$ の時間反転過程 $\{Y_t\}_{t\in[0,T]}$ は, $X_T\approx \mathcal{N}(0,I)$ から時間の経過とともに
  ...
  %---------7-------------
    ...
    \begin{equation*}
        \left\{ \,
            ...
            & Y_0 \stackrel{\mathrm{d}}{=} X_T\approx \mathcal{N}(0,I)
            \end{aligned}
        \right.
    \end{equation*}
    \begin{itemize}
      \item 上記のSDEを最終時刻 $T$ までシミュレーションすると, $Y_T\sim P_{\text{data}}$ からのサンプリングが得られることがわかった. しかし, 現実的には以下の3つの点が問題となる.
      \begin{itemize}
        \item 初期分布 $\mathcal{L}(X_T)$ は $T$ を大きくとると $\mathcal{N}(0,I)$ に近づくが, 有限の時刻 $T$ をとる必要があるため, 厳密には $\mathcal{N}(0,I)$ とは一致せず, 初期分布からのサンプリングが得られない.
        \item SDEのドリフト項に現れるスコア関数 $\nabla \log{p_{T-t}(Y_t)}$ は未知の関数なので, 明示的に得ることができない.
        \item 非線形SDEを連続時間でシミュレーションすることはできない.
      \end{itemize}
    \end{itemize}
  \end{frame}
  %---------8-------------
    ...
    \end{equation*}
    \begin{itemize}
      \item 前頁の3つの問題点に対して, 次のように対処したSDE $\hat{Y}$ を考える.
      \begin{itemize}
        \item 初期分布を, $\hat{Y}_0\sim \mathcal{N}(0,I)$ にする.
  ...
  \end{frame}
    ...
    \end{equation}
    \begin{itemize}
      \item (4) のSDEの解を $\{Y_t\}_{t\in[0,T]}$, (5) のSDEの解を $\{\hat{Y}_t\}_{t\in[0,T]}$ とする.
      \item 確率測度に対する何らかの距離 $\rho$ を用いて, $\rho(P_{\text{data}},\mathcal{L}(\hat{Y}_T))=\rho(\mathcal{L}(Y_T),\mathcal{L}(\hat{Y}_T))$ を評価することが重要な課題である.
    \end{itemize}
  \end{frame}
    %---------10-------------
