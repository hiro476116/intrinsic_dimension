...
\begin{frame}{主結果 : 定理}
      \begin{theorem}
        ...
        \begin{align*}
            ...
            &\leqq C\Biggl(
            \underbrace{T\sqrt{\frac{k\log{k}}{N}}}_{\text{離散化誤差}}
...
\begin{frame}[t]{設定：Tweedieの公式}
  ...
  となる. よって, $\{Y_t\}_{t\in[0,T]}$ が従うSDEは次のように変形できる.
      ...
      \begin{equation*}
        \left\{ \,
            \begin{aligned}
            & dY_t = \frac{2e^{-(T-t)}\mathbb{E}[Y_T\mid Y_t]
              -(1+e^{-2(T-t)})Y_t}{1-e^{-2(T-t)}}dt
            +\sqrt{2}dW_t \\
            & Y_0 \stackrel{\mathrm{d}}{=} X_T
            \end{aligned}
        \right.
  ...
  として, 各 $t\in[t_n,t_{n+1}]$ において実際にシミュレーションするSDEを次のように定める.
      \begin{equation*}
        \left\{ \,
            \begin{aligned}
            & d\hat{Y}_t =
              \frac{2e^{-(T-t)}\hat{\mu}_{T-t_n}(\hat{Y}_{t_n})
              -(1+e^{-2(T-t)})\hat{Y}_t}{1-e^{-2(T-t)}}dt
            +\sqrt{2}dW_t \\
...
\begin{frame}{証明の概要 : 離散化誤差を評価}
      \begin{lemma}
        ...
        \begin{align*}
          ...
          -\E\left[\operatorname{Tr}\left(\operatorname{Cov}\left(Y_T\mid Y_t\right)\right)\right]
          \Bigr)\,dt\\[0.2em]
          &\leqq C_2\log N_{\mathrm{cover}}\left(
