 ...
 %2
  ...
  \begin{frame}{拡散モデルの収束解析について}
    \begin{itemize}
      ...
      \item 近似的に生み出すデータが従う確率測度を $\hat{p}$ とした時に,何かしらの確率測度の距離 $\rho$ を用いて $\rho(p_{data},\hat{p})$ を評価することが収束解析の主な目的である.
      \item 先行研究では, 距離 $\rho$ として主にKLダイバージェンスとWasserstein距離が用いられている.
    \end{itemize}
  ...
  \begin{frame}{拡散モデルの収束解析について(2-Wasserstein距離)}
    \begin{itemize}
        ...
        \item しかし, これらの先行研究ではデータ分布 $p_{data}$ へのlog-concave性の仮定など, 一般的ではない仮定が置かれており, KLと比較して発展途上である.
        \item 
    \end{itemize}
  \end{frame}
  ...
  \begin{frame}{データの内在次元について}
    \begin{itemize}
        \item Huang et alのイントロにいい感じのものがある
    \end{itemize}
  \end{frame}
  %---------6-------------
